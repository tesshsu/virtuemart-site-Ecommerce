...
//		$recurringDesc .= vmText::_('VMPAYMENT_PAYPAL_SUBSCRIPTION_TERM').': '.$this->_method->subscription_term . '<br />';
	...
	public function processIPN ($paypal_data, $payments) {
		...
		// check that the remote IP is from Paypal. MODIF PAR BRUNO CAMELEONS
	/*	if (!$this->checkPaypalIps($paypal_data)) {
		...
		if ($paypal_data['txn_type'] == 'subscr_cancel') {
			...
			if (strcmp($paypal_data['payment_status'], 'Completed') == 0) {
				...
				//FIX MAIL BRUNO CAMELEON
				if ($this->order['details']['BT']->order_language == 'fr-FR') {
					$isEnglish = '';
				} else {
					$isEnglish = 'EN_';
				}

				$order_history['comments'] = vmText::sprintf('VMPAYMENT_'.$isEnglish.'PAYPAL_PAYMENT_STATUS_CONFIRMED', $this->order['details']['BT']->order_number);
