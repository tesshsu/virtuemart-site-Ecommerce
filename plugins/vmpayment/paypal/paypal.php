	...
	function getExpressCheckoutHtml( $cart) {
		...
		foreach ($this->methods as $this->_currentMethod) {
			if ($this->_currentMethod->paypalproduct == 'exp') {
				...
				//Issue #70 Removed paypal express button
...
//			$this->customerData->clear();
	...
	function plgVmOnPaymentNotification() {
		...
		//BRUNO TEST
		//mail('bruno@cameleons.com', 'nouvelle notification paypal', $virtuemart_order_id);
		...
		//mail('bruno@cameleons.com', 'interface content', print_r($order_history) );

		if (!$order_history) {
			return false;
			//mail('bruno@cameleons.com', 'paypal false ', $virtuemart_order_id);
			
		} else {
			//mail('bruno@cameleons.com', 'paypal true ', $virtuemart_order_id);
			...
			$paypalInterface->debugLog('plgVmOnPaymentNotification order_number:' . $order_number . ' new_status:' . $order_history['order_status'], 'plgVmOnPaymentNotification', 'debug');
				...
				//BRUNO TEST
			//mail('bruno@cameleons.com', 'STATUT', $order_history['order_status']);
		...
		//BRUNO TEST
		//mail('bruno@cameleons.com', 'paypal interface ok ', $virtuemart_order_id);
