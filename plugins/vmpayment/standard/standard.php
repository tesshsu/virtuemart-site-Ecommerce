...
class plgVmPaymentStandard extends vmPSPlugin {
	...
	function plgVmConfirmedOrder ($cart, $order) {
		...
		//BRUNO CAMELEON : Changement order id
		...
		$html = $this->renderByLayout('post_payment', array(
			'order_number' =>$order['details']['BT']->virtuemart_order_id,
...
*/
	...
	function plgVmOnUserInvoice ($orderDetails, &$data) {
		...
		if ($orderDetails['order_salesPrice']==0.00) {
			//BRUNO CAMELEONS : Modifier id
			$data['invoice_number'] = 'reservedByPayment_' . $orderDetails['virtuemart_order_id']; // Nerver send the invoice via email
		}
