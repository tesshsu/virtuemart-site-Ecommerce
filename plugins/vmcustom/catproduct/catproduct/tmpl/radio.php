...
<tr>
	...
	<th class="cell_quantity"><?php echo JText::_('CATPRODUCT_TABLE_RADIO_TITLE'); $colspan += 1; ?>
	<?php } else { ?>
...
/*	$price_per_unit = 'NA';
	  ...
	  if (empty($product['child']['catproduct_inline_row'])) {		
		...
		if ($check_stock == 1 && $product['child']['product_in_stock'] > 0 || $check_stock == 0){
			...
			if (!VmConfig::get('use_as_catalog', 0)  ) {
				if ($see_price) {
					echo '<td class="cell_quantity" data-title="'.JText::_('CATPRODUCT_TABLE_RADIO_TITLE').'" ><input name="virtuemart_product_id[]['.$group_id.']" id="virtuemart_product_id'.$i.'" type="radio" value="'.$product['child']['virtuemart_product_id'].'" 
					 data-groupid="'.$group_id.'"  /></td>';
				} else {
					echo '<td class="cell_quantity"  data-title="'.JText::_('COM_VIRTUEMART_PRODUCT_ASKPRICE').'" ><input type="hidden" id="quantity_'.$i.'" name="quantity[]" value="0">';
	...
	//$i=0;
		...
		if ($show_qty = "1") {
		echo '	<td colspan="'.((isset($parametri["addtocart_button"]) && $parametri["addtocart_button"] == "foreach")?$colspan-2:$colspan-1).'" align="right">'.JText::_('CATPRODUCT_TABLE_QUANTITY').'</td>
				  <td class="cell_quantity">
				  <span class="quantity-box" style="margin-left:20px !important;"><input class="quantity-input js-recalculate" size="2" name="quantity[]" value="'.$def_group_qty.'" type="text" data-groupid="'.$group_id.'"  data-group_def_qty="'.$def_group_qty.'" ></span>
			...
			echo '<input class="quantity-input js-recalculate" name="quantity[]" value="'.$def_group_qty.'" type="hidden"  data-groupid="'.$group_id.'" data-group_def_qty="'.$def_group_qty.'">';
		}
				
		// show row addtocartbutton
		echo $see_price?$currency->showAddtocartButton('foreach', $parametri, $colspan, 0, 1, $classes):'';
