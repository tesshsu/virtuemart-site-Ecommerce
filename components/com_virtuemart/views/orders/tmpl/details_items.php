...
<?php //FLORIAN CAMELEONS : calcul de la présence de remises pour effacer la colonne
	$colDiscount=0;
 ...
 ?>
<table width="<?php echo $widthTable ?>%" cellspacing="0" cellpadding="0" border="0" style="margin-bottom: 50px;">
	<tr align="left" class="sectiontableheader">
		<th align="left" width="5%"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_SKU') ?></th>
		<th align="left" width="30%" ><?php echo vmText::_('COM_VIRTUEMART_PRODUCT_NAME_TITLE') ?></th>
		<th align="center" width="10%"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_PRODUCT_STATUS') ?></th>
		<th align="right" width="10%" style="text-align:right"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_PRICE') ?></th>
		<th align="left" width="5%" style="text-align:right"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_QTY') ?></th>
		<?php if ( $this->orderdetails['details']['BT']->order_billTaxAmount > 0) { ?>
		<th align="right" width="10%" style="text-align:right"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_PRODUCT_TAX') ?></th>
		  <?php } else { ?>
		 <th align="right" width="10%" style="text-align:right"><?php echo vmText::_(' ') ?></th>
		  <?php } ?>
        <!-- FLORIAN CAMELEONS : ajout du if qui supprime la colone -->
        <?php if($this->orderdetails['details']['BT']->order_billDiscountAmount > 0) { ?>
        <th align="right" width="11%" style="text-align:right"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_SUBTOTAL_DISCOUNT_AMOUNT') ?></th>
        <th align="right" width="10%" style="text-align:right"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_TOTAL') ?></th>
        <?php } else { ?>
        <td align="right" width="11%" style="text-align:right"><strong><?php echo vmText::_(' ') ?></strong></td>
        <td align="right" width="11%" style="text-align:right"><strong><?php echo vmText::_('COM_VIRTUEMART_'.$isEnglish.'ORDER_PRINT_TOTAL') ?></strong></td>
        <?php } ?>
	</tr>
<?php
	foreach($this->orderdetails['items'] as $item) {
...
?>
		<tr valign="top">
			...
			<td align="right" >
				...
				<td align="right" class="priceCol">
					<?php if ( $this->orderdetails['details']['BT']->order_billTaxAmount > 0) {
						echo "<span  class='priceColor2'>".$this->currency->priceDisplay($item->product_tax ,$this->user_currency_id, $qtt)."</span>";
					} ?>
				</td>
...
?>
 <tr class="spacer" style="border-bottom: 1px solid #ddd;"><td style="padding-bottom: 10px;" ></td></tr>
 <tr class="sectiontableentry1">
			<td colspan="6" align="right" style="padding-top: 10px;"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_PRODUCT_PRICES_TOTAL'); ?></td>
...
<?php  } ?>
	...
	<tr>
		<?php if( $this->orderdetails['details']['BT']->order_shipment > 0 ) { ?>
		<td align="right" class="pricePad" colspan="6"><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_SHIPPING') ?></td>
			...
			<?php if ( VmConfig::get('show_tax')) { ?>
				<td align="right"><?php echo "<span  class='priceColor2'>".$this->currency->priceDisplay($this->orderdetails['details']['BT']->order_shipment_tax, $this->user_currency_id)."</span>" ?></td>
                                <?php } ?>
				<td align="right">&nbsp;</td>
				<td align="right"><?php echo $this->currency->priceDisplay($this->orderdetails['details']['BT']->order_shipment+ $this->orderdetails['details']['BT']->order_shipment_tax, $this->user_currency_id); ?>
		</td>
		<?php } ?>
	</tr>
...
<tr>
	...
	<tr>
		...
		<td align="right" colspan=2 ><strong><?php echo $this->currency->priceDisplay($this->orderdetails['details']['BT']->order_total, $this->user_currency_id); ?></strong></td>
	</tr>
