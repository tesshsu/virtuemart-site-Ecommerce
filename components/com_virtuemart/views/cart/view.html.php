		...
		if($this->cart->user->virtuemart_user_id){
			...
			foreach($this->cart->user->userInfo as $userInfo){
				...
				if($address->address_type=='BT'){
					...
					. '" rel="nofollow">'.$address->address_type_name.'</a>&nbsp;&nbsp;&nbsp;<a href="'.JRoute::_ ('index.php?option=com_virtuemart&view=user&task=removeAddressST&virtuemart_user_id[]=' . $_addressList[$_i]->virtuemart_user_id . '&virtuemart_userinfo_id=' . $address->virtuemart_userinfo_id, $useXHTTML, $useSSL ). '" >'.'<i class="fa fa-times-circle-o"></i>'.vmText::_('COM_VIRTUEMART_USER_DELETE_ST').'</a></br>';					
				    $addressList[] = $address;
				}
			}
