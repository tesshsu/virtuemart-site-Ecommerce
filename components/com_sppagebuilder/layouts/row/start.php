...
$equal_height = (isset($options->columns_equal_height) && $options->columns_equal_height ) ? $custom_class .= ' sppb-equalize-columns' : '';
...
if ( (isset($options->title) && $options->title) || (isset($options->subtitle) && $options->subtitle) ) {
	...
	$title_section_css  = (isset($options->title_section_margin) && $options->title_section_margin) ? 'margin: ' . $options->title_section_margin . ';' : '';
	$title_section_css .= (isset($options->title_section_padding) && $options->title_section_padding) ? 'padding: ' . $options->title_section_padding . ';' : '';

	if($title_section_css) {
		$doc->addStyledeclaration('#sp-page-builder #' . $row_id . ' .sppb-section-title {'. $title_section_css .'}');
	}
