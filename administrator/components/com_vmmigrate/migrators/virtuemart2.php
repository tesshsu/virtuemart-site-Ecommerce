...
class VMMigrateModelVirtuemart2 extends VMMigrateModelBase {
...
//		}
		...
		$this->logDebug($this->default_language,'Default language');
		$this->logDebug($this->default_language_ext,'Default language_ext');
	}
	...
	private function getSrcConfigValue($key,$default='') {
		...
		if (version_compare($srcVersion,'2.0','<') || version_compare($srcVersion,'3.0','>=')) {
			return $default;
		}
		if ($key) {
	...
	public static function getMessages() {
		...
		if (version_compare($srcVersion,'2.0','<') || version_compare($srcVersion,'3.0','>=')) {
			$messages['error'][] = JText::_('VIRTUEMART2_WRONG_VERSION_SRC');
		}
		return $messages;
	}
	...
	public function getSrcVersion() {
		...
		if ($this->source_filehelper->FileExists('/administrator/components/com_virtuemart/version.php' )) { 
			...
			try {
				if (!class_exists('vmVersionSrc2')) {
					$buffer = str_replace('vmVersion','vmVersionSrc2',$buffer);
				...
				$VMVERSION = new vmVersionSrc2();
				if (isset($VMVERSION->RELEASE)) {
					$version = $VMVERSION->RELEASE;
				} else {
					$version = vmVersionSrc2::$RELEASE;
				}
    ...
    public function vm_vendor() {
        ...
        foreach ($items as $i => $item) {
			...
			$records_lang = array();
			foreach ($this->getLanguagesExt() as $language_ext) {
				$records_lang[$language_ext] = $this->get_record($table.'_'.$language_ext,$pk.'='.$srcid);
			}
	...
	public function vm_tax_calc() {
        ...
        foreach ($items as $i => $item) {
			...
			if (version_compare($srcVersion,'2.0.18','>')) {
	...
	public function vm_shipment_method() {
        ...
        foreach ($items as $i => $item) {
			...
			if (version_compare($this->getVmVersion(),'2.9','>') && $record->shipment_element == 'weight_countries') {
	...
	public function vm_manufacturer_category() {
        ...
        foreach ($items as $i => $item) {
			...
			foreach ($this->getLanguagesExt() as $language_ext) {
				$translation = $this->get_record($table.'_'.$language_ext,$pk.'='.$srcid);
				if (!$translation) {
					$translation = $this->get_record($table.'_'.'en_gb',$pk.'='.$srcid);
				}
				$records_lang[$language_ext] = $translation;
			}
	...
	public function vm_product_stock() {
        $pk = 'product_id';
        $excludeids = $this->getAlreadyTransferedIds();			// Load already transfered items
		$items = $this->getItems2BTransfered('virtuemart_products',$pk,$excludeids,'','product_parent_id ASC');	//Get a list of source objects to be transfered
        if (!$items) {
			$this->logInfo(JText::_('NOTHING_TO_TRANSFER'));
            return false;
        }
	...
	private function getNbOrderedItems($product_id) {
		...
		$query->select('sum(product_quantity) as counter')
			...
			->where('virtuemart_product_id='.$this->destination_db->q($product_id))
	...
	public function vm_customfields() {
        ...
        foreach ($items as $i => $item) {
			...
			if (version_compare($this->getVmVersion(),'2.9','>')) {
				...
				if ($record->field_type=='V') {
					$record->field_type		= 'S';
				} else if ($record->field_type=='P') {
					$record->field_type		= 'G';
				}
				if ($record->field_type == 'S' && $isCart) {
					$record->is_list = 0;
				}
			}
	...
	public function vm_orders() {
		...
		VMMigrateHelperDatabase::copyTableStructure($this->source_db,'#__virtuemart_order_items');
        ...
        foreach ($items as $i => $item) {
			...
			if (version_compare($this->getVmVersion(),'2.9.9.4','>=') && isset($record->customer_note)) {
			...
			try {
				...
				foreach ($items as $child_record) {
					...
					if (version_compare($this->getVmVersion(),'2.9.9.4','>=')) {
						$child_record->product_attribute = $this->migrateAttributeFormat($child_record->product_attribute);
					}
	...
	private function migrateAttributeFormat($attributes) {
		$json = @json_decode($attributes);
		if($json){
			$vars = get_object_vars($json);
			foreach($vars as $customfieldid=>$value){
				if(!empty($customfieldid)){
					$virtuemart_custom_id = $this->getCustomId($customfieldid);
					$custom_element = $this->getCustomElement($virtuemart_custom_id);
					$vars2 = get_object_vars($value);
					if ($custom_element && isset($vars2[$custom_element])) {
						$newAttributes[$virtuemart_custom_id][$customfieldid] = $vars2[$custom_element];
					} else {
						$newAttributes[$virtuemart_custom_id][$customfieldid] = $value;
					}
				}
			}
			return json_encode($newAttributes);
		} else {
			return $attributes;
		}
	}
	
	private function getCustomId($virtuemart_customfield_id) {
		$query = $this->destination_db->getQuery(true);
		$query->select('virtuemart_custom_id')
			->from('#__virtuemart_product_customfields')
			->where("virtuemart_customfield_id = ".$this->destination_db->q($virtuemart_customfield_id));
		$this->destination_db->setQuery($query);
		$virtuemart_custom_id = $this->destination_db->loadResult();
		return $virtuemart_custom_id;
	}

	private function getCustomElement($virtuemart_custom_id) {
		$query = $this->destination_db->getQuery(true);
		$query->select('custom_element')
			->from('#__virtuemart_customs')
			->where("virtuemart_custom_id = ".$this->destination_db->q($virtuemart_custom_id));
		$this->destination_db->setQuery($query);
		$custom_element = $this->destination_db->loadResult();
