...
foreach ($this->slideshow as $i => $item) : ?>
	 <tr  style="height: 38px; <?php if($item->id %2 == 0) echo "background:#F0F0F0 "?>">
            ...
            <td style="height: 41px;">
            <?php  $id = $item->id; ?>
                <a href="<?php echo JURI::base().'index.php?option=com_slideshow&view=slideshow&id='.$id ?>"> 
            ...
            <?php  $id = $item->id; ?>
                    <span>
                   [huge_it_slideshow_id="<?php echo $this->escape($item->id); ?>"] 
                    </span>
		</td>
