...
class SlideshowModelGeneral extends JModelAdmin {
    public function getTable($type = 'General', $prefix = 'SlideshowTable', $config = array()) {
    ...
    function save($data) {
        ...
        $db = JFactory::getDbo();
        echo $data['params[slideshow_title_font_size]'];
    ...
    function  apply1(){
        ...
        $query = $db->getQuery(true);
        $query->update('#__huge_itslideshow_params')->set('value="' . $data['params'][$name] . '"')->where('id=' . $id);
        $db->setQuery($query);
        $db->execute();
        }
    }
