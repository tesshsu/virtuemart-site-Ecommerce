...
if ($this->allowRating || $this->allowReview || $this->showRating || $this->showReview) {
	...
	$ratingWidth = $maxrating*10; 
	for( $num = 0; $num<=$maxrating; $num++ ) {
		$stars[] = '
				<span title="'.(vmText::_( "COM_VIRTUEMART_RATING_TITLE" ).$num.'/'.$maxrating).'" class="vmicon ratingbox" style="display:inline-block;width:'. 10*$maxrating.'px;">
					<span class="stars-orange" style="width:'.(10*$num).'px">
					</span>
				</span>';
	}
...
";
	...
	<?php
		...
		echo '<strong>'.vmText::_( 'COM_VIRTUEMART_DEAR' ).$this->user->name.',</strong><p>';
		echo vmText::_( 'COM_VIRTUEMART_REVIEW_ALREADYDONE' ).'</p>';
	}
}


if ($this->showReview) {
    $count_review = count($this->rating_reviews);
	?>
	<h4><?php echo $count_review.vmText::_ ('COM_VIRTUEMART_REVIEWS') ?></h4>
			...
			foreach ($this->rating_reviews as $review) { 
				...
				if (!empty($this->rating_reviews) && $review->published) {
					...
					<li class="review-item"> 
                        <div class="meta">
                            <strong><?php echo $review->customer ?></strong> - <?php echo JHtml::date ($review->created_on, vmText::_ ('DATE_FORMAT_LC')); ?>
                        </div> 
						<?php echo $stars[(int)$review->review_rating] ?>
						<div class="description"><p><?php echo $review->comment; ?></p></div>						
					</li>
					<?php
				}
				$i++;
